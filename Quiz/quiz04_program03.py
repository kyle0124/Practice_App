...
df2 = getData(code, date_start, datetime.today().date())  

# 그래프 출력에 유리한 형태로 데이터프레임 변환.
ser2 = df2['Close'].reset_index(drop=True)      # Pandas의 Series 객체.

# ES 모델생성 및 학습.
model = ExponentialSmoothing(ser2, trend='add', seasonal='add', seasonal_periods=5).fit() 
